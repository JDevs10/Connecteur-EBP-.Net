...
namespace ConsoleApplication1.Helpers
{
    ...
    public static class Utils
    {
        ...
        public static List<ConsoleApplication1.Application> GetCompatibleProducts()
        {
            ...
            try
            {
                List<ConsoleApplication1.Application> applications = new List<ConsoleApplication1.Application>();
                    ...
                    foreach (string subKeyName in key.OpenSubKey(keyName).GetSubKeyNames())
                    {
                        ...
                        if (subSubKey.GetValue("Path") != null && subSubKey.GetValue("Path").ToString().ToLower().Contains(productType))
                        {
                            if (!File.Exists(subSubKey.GetValue("Path").ToString()))
                                continue;
                            ConsoleApplication1.Application application = new ConsoleApplication1.Application();
