...
{
    public partial class Import_facture : Form
    {
        private static string filename = "";
        private static string ordersFilename = string.Format(@"{0}\imporFacture.txt", Path.GetTempPath());// string.Format(@"C:\Documents and Settings\OTHMAN\Bureau\resultat11.txt");

        public Import_facture()
        {
            InitializeComponent();
        }
        ...
        private void exportCustomersButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (lines[1].Split(';')[0] == "DEMAT-HD1" && lines[1].Split(';').Length == 63)
                {
                    NumCommande = lines[1].Split(';')[3];
                    if (NumCommande.Length > 9)
                    {
                        MessageBox.Show("Numéro de facture doit être < 10", "Erreur de lecture !!",
                    ...
                    if (NumCommande == "")
                    {
                        MessageBox.Show("Le champ numéro de facture est vide.", "Erreur !!",
                                                                           MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        return;
                    }
                    ...
                    {
                        MessageBox.Show("Numéro de facture n'est pas valide.", "Erreur !!",
                                                                                                  MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        return;
                    }
                    ...
                    {
                        MessageBox.Show("Numero de facture déja enregistrer : " + NumCommande, "Erreur !!",
                              MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        return;
                    }

                    //order.codeClient = lines[0].Split(';')[2];
                    if (lines[2].Split(';')[0] == "DEMAT-HD2" && lines[2].Split(';')[7] == "")
                    {
                    ...
                    order.ThirdId = GetClientId(lines[2].Split(';')[7]);
                    ...
                    if (lines[1].Split(';')[6].Length == 8)
                    {
                        string date = lines[1].Split(';')[6];

                        if (!IsNumeric(date))
                        {
                            MessageBox.Show("Date de la facture n'est pas valide.", "Erreur !!",
                                MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                            return;
                        }
                    ...
                    {
                        MessageBox.Show("Date de la facture est incorrecte", "Erreur de lecture !!",
                                               MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        return;
                    }
                        ...
                        switch (tab[0])
                        {
                            case "DEMAT-LIN":
                                if (tab.Length == 51)
                                {
                                    ...
                                    decimal quant = Convert.ToDecimal(tab[15].Replace(".", ","));
                                    ...
                                    //}
                                    string test = tab[22].Replace(",", ".");
                                    line.ItemPrice = Decimal.Parse(tab[22].Replace(",", "."), NumberStyles.AllowCurrencySymbol | NumberStyles.AllowDecimalPoint | NumberStyles.AllowThousands, CultureInfo.InvariantCulture);
                    ...
                    if (order.ThirdId != "")
                    {
                        ...
                        {
                            MessageBox.Show("Aucun ligne de facture enregistré.", "Erreur de lecture !!",
                                         MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                            return;
                        }
                ...
                if (order.ThirdId != null)
                {
                    ...
                    try
                    {
                        ...
                        string importArguments = string.Format(CommandLinesHelper.ImportFactures, ordersFilename);
                        //MessageBox.Show(ordersFilename);
                        //Exécution de la ligne de commande
                        Utils.LaunchCommandLineProcess(importArguments);
                        // Creer dossier sortie "LOG Directory" --------------------------
                        var dirName = string.Format("LogEBP(manuelle) {0:dd-MM-yyyy}", DateTime.Now);
