...
namespace ConnecteurEBP.Forms
{
    public partial class BonLivraison : Form
    {
        ...
        private List<DocumentVente> GetBonLivraisonFromDataBase(string codeClient)
        {
            ...
            using (SqlConnection sqlConnection = Utils.CreateSqlConnection())
            {
                try
                {
                    sqlConnection.Open();
                    //Exécution de la requête permettant de récupérer les articles du dossier
                    using (SqlCommand cmd = new SqlCommand(QueryHelper.getBonLivraisonClient(codeClient), sqlConnection))
                    {
                        using (IDataReader reader = cmd.ExecuteReader())
                        {
                           while(reader.Read())
        ...
        private void ExportFacture()
        {
            try
            {
                ...
                var fileName = string.Format("BONLIVRAISON_{0:ddMMyyyy_HHmmss}.csv", DateTime.Now);
                ...
                using (StreamWriter writer = new StreamWriter(textBox1.Text+@"\"+fileName, false, Encoding.Default))
                {
                    ...
                    for (int i = 0; i < FacturesAExporter.Count; i++)
                    {
                        ...
                        { // Return la commande d'origin   
                            string res = GetCommandeFacture(FacturesAExporter[i].Id);
                            if(res != null)
                            {
                                tab = res.Split(';');
                            }
                        ...
                        writer.WriteLine("DESREF;;;;;;" + FacturesAExporter[i].reference.Replace("Document importé n° ", "").Replace(" (prix article différent)", "") + ";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;");
                ...
                MessageBox.Show("Nombre bon de livraison : " + FacturesAExporter.Count , "Information !!",
                ...
                Thread backgroundThread = new Thread(
                    new ThreadStart(() =>
                    {
                        ...
                        if (itemsDataGridView.InvokeRequired)
                        {
                            ...
                            {
                                itemsDataGridView.DataSource = GetBonLivraisonFromDataBase(customer.Id);
                                ...
                                itemsDataGridView.Columns["DocumentDate"].HeaderText = "Date Bon Livraison";
        ...
        private void itemsDataGridView_SelectionChanged(object sender, EventArgs e)
        {
            ...
            orderLinesDataGridView.Columns["DocumentDate"].HeaderText = "Date Bon Livraison";
